...
trigger QuoteLineItemTrg on QuoteLineItem (before insert, after insert, after update, before delete, after delete, before update) 
{
    ...
    if (Trigger.isAfter && (Trigger.isInsert || Trigger.isUpdate || Trigger.isDelete)){    
        ...
        for (QuoteLineItem qli : Trigger.isDelete?Trigger.Old:Trigger.New)
        {
            if (Trigger.isUpdate)  
            {                   
                setProject.add(qli.Project_Id__c);
        ...
        if (setQuoteLineId != null && setQuoteLineId.size() > 0) {
            for (QuoteLineItem qli : [select Id, UnitPrice, ListPrice, Everyday_Price__c, QuoteId, LineNumber, Quote.Opportunity.RecordType.Name, Product2.ProductCode, Unique_ID__c, IsCloned__c, Quote.Discount__c, Approved_Price__c, Product2.Brand__c, Quote.Active_Quote__c from QuoteLineItem where Id in :setQuoteLineId ])
            {
                ...
                if (!qli.isCloned__c){
                    /*
                    if (qli.Everyday_Price__c != null && qli.Everyday_Price__c > 0 && (qli.UnitPrice==null || qli.UnitPrice==0) ) {                        
        ...
        if(!setProject.isEmpty()){
            ProjectTrgHandler.UpdateWholeSalers(setProject);
        }
        /*
system.debug('hb::'+setQuoteIdWholeProjcetCSV);
if(!setQuoteIdWholeProjcetCSV.isEmpty()){
map<id,Project__c> mapProject = new map<Id,Project__c>();
map<id,Opportunity> mapOpportunity = new map<Id,Opportunity>();
set<string> setWholes = new Set<string>();
for (QuoteLineItem qli : [select Id, Product2.Brand__c,Wholesalers__c,quote.opportunityId,Project_Id__c from QuoteLineItem where QuoteId in :setQuoteIdWholeProjcetCSV and Quote.Active_Quote__c = true ])
{                

if(string.isNotBlank(qli.Wholesalers__c)){
Project__c aProject = new Project__c();
if(mapProject.containsKey(qli.Project_Id__c)){
aProject = mapProject.get(qli.Project_Id__c);
}
else{
aProject.Id = qli.Project_Id__c;
}
String Wholesaler_csv = '';
if(string.isNotBlank(aProject.Wholesalers_CSV__c)){
Wholesaler_csv = aProject.Wholesalers_CSV__c.trim();                        
for(string t:Wholesaler_csv.split(',')){
setWholes.add(t.trim());
}

}
for(string t:qli.Wholesalers__c.split(', ')){
setWholes.add(t.trim());
}

List<String> tempWS = new List<String>();
tempWS.addAll(setWholes);                    
Wholesaler_csv = string.join(tempWS, ',');
Wholesaler_csv = Wholesaler_csv.mid(0, 255);
aProject.Wholesalers_CSV__c = Wholesaler_csv.removeStart(',').removeEnd(',').trim();
if(string.isnotblank(aProject.Wholesalers_CSV__c)){
mapProject.put(qli.Project_Id__c,aProject);
}

}      
if(string.isNotBlank(qli.Product2.Brand__c)){
Opportunity anOpp = new Opportunity();
if(mapOpportunity.containsKey(qli.quote.opportunityId)){
anOpp = mapOpportunity.get(qli.quote.opportunityId);
}
else{
anOpp.Id = qli.quote.opportunityId;
}
String brands = '';
brands = anOpp.Brand__c;
if(string.isnotblank(brands)){
brands = ';'+brands+';';
if(!brands.contains(';'+qli.Product2.Brand__c+';')){
brands = brands+qli.Product2.Brand__c;
}                        
}
else{
brands = qli.Product2.Brand__c;
}
brands = brands.mid(0, 255);
anOpp.Brand__c = brands.removeStart(';').removeEnd(';');                    
if(string.isnotblank(anOpp.Brand__c)){
mapOpportunity.put(qli.Project_Id__c,anOpp);
}
