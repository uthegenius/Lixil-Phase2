  ...
  //  {
        ...
        if (Trigger.isUpdate && Trigger.isAfter)
        {
            ...
            if ( setShippedOpp.size() > 0 )
            {
                ...
                for (Opportunity opp : [select Id, Responsible__c, RecordType.DeveloperName, StageName,
                                        (Select Id
                                        From Tasks
                                        Where RecordType.Name='Display'
                                        and Subject = :Label.Shipped_Opportunity_Task_Subject) from opportunity where Id in :setShippedOpp and RecordType.DeveloperName = 'Display' and Responsible__c != null])
                {
